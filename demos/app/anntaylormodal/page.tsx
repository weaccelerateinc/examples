  ...
  const handleSubmit = async (e: FormEvent<HTMLFormElement>) => {
    ...
    if (selectedCard) {
      ...
      try {
        ...
        router.push(
          `/anntaylormodal/payment/confirmation?` +
                ...
                <div className="p-4 space-y-4">
                  ...
                  {selectedPayment === "card" && accelLoaded && isLoggedIn && (
                    <div className="pt-1 space-y-3">
                      {selectedCard ? (
                        <div className="flex items-center justify-between p-3 border border-[#979797] bg-gray-50">
                          <span className="text-sm text-black">Card selected</span>
                          <button
                            type="button"
                            onClick={() => window.accelerate.openWallet()}
                        ...
                        <button
                          type="button"
                          onClick={() => window.accelerate.openWallet()}
                          className="w-full border border-black py-3 text-sm font-semibold text-black hover:bg-gray-50 rounded-sm"
                        >
