...
function PaymentContent() {
  ...
  useEffect(() => {
    if (window.accelerate && Object.keys(window.accelerate).length > 0) {
      try {
        if (typeof window.accelerate.init === 'function' && 
            typeof window.accelerate.login === 'function') {
          console.log("Accelerate already initialized");
          setAccelerateLoaded(true);
        }
      } catch {
        console.log("Accelerate not initialized yet");
      }
    }
  }, []);
  ...
  return (
    <div className="flex overflow-hidden flex-col bg-white">
      ...
      <Script
        ...
        onReady={() => {
          if (!window.accelerate?.init || !accelLoaded) {
            console.log("p2.onReady - initializing Accelerate");
