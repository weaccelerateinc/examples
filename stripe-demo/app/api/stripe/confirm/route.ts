...
export async function POST(request: NextRequest) {
  ...
  // const intent = await stripeClient.paymentIntents.retrieve(data.paymentIntentId);
  // const myCart = get_cart(checkoutId);
  // if (myCard.amount != intent.amount) throw new Error("Problems")
  //

  try {
    const confirmation = await stripeClient.paymentIntents.confirm(data.paymentIntentId);
